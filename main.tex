...
\begin{theorem}[{\cite[7.2.2]{Hirzebruch}, Thom}] If $i\not\equiv 0\pmod{4}$, then $\Omega_i$ is finite. We have that $\Omega^{4k}$ has free part $\Z^{\pi(k)}$ and a finite torsion part.
\end{theorem}

\begin{example} $\Omega_0\cong \Z$, as a compact oriented $0$-manifold is a finite set of signed points and the sum of the signs is a bordism invariant.
\end{example}

\begin{example} $\Omega^1\cong 0$, since every compact $1$-manifold is a disjoint union of circles, and all such manifolds bound. 
\end{example}

\begin{example} $\Omega_2\cong 0$, as every compact oriented $2$-manifold bounds. Can you find an example of one that doesn't bound if we dropped the word oriented?
\end{example}
