...
\begin{proposition} Let $X$ be a space, $\mathcal{U}$ an open cover, and $F$ a sheaf. Suppose that the sheaf cohomology satisfies $H^1(U_{i_0 \cdots i_q}, F_{|U_{i_0 \cdots i_q}}) = 0$ for all finite intersections of elements in our cover. Then for any injective resolution of sheaves
\begin{align*}
...
\begin{definition}\label{def:Leray-cover} Let $X$ be a space and $\mathcal{U}$ an open cover. We say $F$ is \emph{a Leray cover for} $\mathcal{U}$ if $H^1(U_{i_0 \cdots i_q}, F_{|U_{i_0 \cdots i_q}}) = 0$ for all finite intersections of open sets in the cover $\mathcal{U}$.
\end{definition}
...
\begin{definition} We say $X$ is \emph{semi-locally contractible} if any open subset $U \subseteq X$ admits an open cover $\left\{ V_i \right\}$ so that the inclusion $V_i \subseteq U$ is homotopic to a constant map.\footnote{That is, $W_i$ need not be contractible as a space, but it is ``contractible in $U$.''}
\end{definition}

\begin{theorem}\label{thm:Cech-cohomology-and-singular-cohomology} 
If $X$ is paracompact, Hausdorff, and semi-locally contractible, and $A$ is any abelian group, there is an isomorphism between sheaf cohomology with constant coefficients and singular cohomology
\begin{align*}
    \check{H}(X, \underline{A}) \cong H^\ast_\text{sing}(X,A).
\end{align*}
\end{theorem}
...
\end{aligned}
\end{equation}
We first claim this is an exact sequence of sheaves (a resolution of the constant sheaf), so we have to verify exactness on stalks. Since $X$ is semi-locally contractible, there is a cofinal system of open neighborhoods $U\ni x$ which are contractible (TODO check this, add to appendix). On such a contractible neighborhood, we get that $\underline{A}(U) = A$ (since $U$ is connected), and we get that all the singular cohomology vanishes, i.e. $H_\sing^i(U,A) = 0$ for $i>0$. In other words, the sequence
\begin{align*}
