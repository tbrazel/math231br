...
\begin{exercise} Argue that the global sections functor
\begin{align*}
...
\end{align*}
We will proceed by induction. The kernel of $d^0$ is $\Gamma(X,F)$ by definition, which is the same as $\check{H}^0(X,F)$ by \Cref{prop:Cech-0-is-global-sections}. This establishes our base case.

Let $K^p := \ker(A^p \to A^{p+1})$. By exactness and the first isomorphism theorem, $A^p/K^p \cong K^{p+1}$, so we get a short exact sequence of sheaves
\begin{align*}
...
\end{align*}
We claim $\check{H}^1(K^{q-1})$ is identified canonically with the $q$th cohomology of the sequence $\left\{ \Gamma(X,A^i) \right\}$. Indeed this follows from looking at the LES attached to $0 \to K^{q-1} \to A^{q-1}\to K^q \to 0$, from which we get
\begin{align*}
    H^0(X,A^{q-1}) \xto{d^{q-1}} H^0(X,K^q) \to H^1(X,K^{q-1}) \to 0.
\end{align*}
Since $H^0(X,K^q) = \ker(d^q)$, we have that $H^1(X,K^{q-1})$ is the $\ker(d^q)$ modulo the image of $d^{q-1}$, hence it is the cohomology of the complex.
\end{proof}

This provides us a new way to compute \v{C}ech cohomology over a paracompact space!

\begin{remark} In general, \emph{sheaf cohomology} is defined as the derived functors of global sections. This makes sense over any space, and admits many key properties (for instance a long exact sequence on cohomology from a short exact sequence of sheaves). However in general it need not agree with \v{C}ech cohomology.
\end{remark}
...
We're going to argue injective sheaves are acyclic. First we need a lemma.
